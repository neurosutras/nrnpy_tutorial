...
def init_worker():
    ...
    print('process: %i; MPI rank/size: %i/%i' %
...
def main(a, verbose):
    ...
    futures = []
    for i in range(context.comm.size - 1):
        future = context.executor.submit(init_worker)
    ...
    futures = []
    for i in range(context.comm.size - 1):
        future = context.executor.submit(modify_a, i)
        futures.append(future)
    for future in futures:
        future.result()
    ...
    if verbose:
        print('Calling report_a via a submit operation returns: %s' % str(results))
