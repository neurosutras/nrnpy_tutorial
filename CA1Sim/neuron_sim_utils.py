...
class HocCell(object):
    def __init__(self, morph_filename=None, mech_filename=None, gid=0):
        """
        :param morph_filename: str : name of .swc file containing morphology
        :param mech_filename: str : name of .pkl file specifying cable parameters and membrane mechanisms
        """
        ...
        self.nodes = {'soma': [], 'axon': [], 'basal': [], 'trunk': [], 'apical': [], 'tuft': [], 'spine': []}
    ...
    def make_section(self, sec_type):
        ...
        if sec_type in ['spine_head', 'spine_neck']:
            self.nodes['spine'].append(node)
        else:
            self.nodes[sec_type].append(node)
...
class SHocNode(SNode2):
    ...
    def __init__(self, index=0):
        ...
        self.content['synapse_locs'] = {'exc': [], 'inh': []}
    ...
    def synapse_locs(self, category):
        """
        Returns a list of potential synapse locations (float) associated with this node.
        Once synapse locs have been marked, the method 'insert_synapses' can be used to insert synapse objects in this
        node at those locations.
        :param category: str in ['exc', 'inh']
        :return: list of hoc objects, type depends on .mod file(s) used to implement synapses
        """
        return self.content['synapse_locs'][category]
...
class QuickSim(object):
    ...
    self.rec_list:
    """
    def __init__(self, tstop=400., cvode=0, dt=None, verbose=1):
    ...
    def export_to_file(self, file_name, simiter=None):
        """
        ...
        :param file_name: str (path to hdf5 file)
        :param simiter: int
        """
        start_time = time.time()
...
class CA1_Pyr(HocCell):
    def __init__(self, morph_filename='EB2-late-bifurcation.swc', mech_filename='043016 Type A - km2_NMDA_KIN5_Pr.pkl',
                 full_spines=False, gid=0):
        ...
        if full_spines:
            ...
            self.mark_excitatory_synapse_locs()
        self.mark_inhibitory_synapse_locs()
    ...
    def mark_excitatory_synapse_locs(self, sec_type_list=None):
        """
        This method marks potential synapse locations within the cell tree following spine density information from
        Erik Bloss & Nelson Spruston. Basal dendrites have no spines until the first branch point, and a higher density
        ...
        if 'basal' in sec_type_list:
            for node in self.basal:
                order = self.get_branch_order(node)
                if order == 2:
                    self.append_synapse_locs(node, 'exc', densities['basal']['2'])
                elif order > 2:
                    self.append_synapse_locs(node, 'exc', densities['basal']['>2'])
        if 'trunk' in sec_type_list:
            for node in self.trunk:
                ...
                if distance >= densities['trunk']['start']:
                    slope = (densities['trunk']['max'] - densities['trunk']['min']) / \
                            (densities['trunk']['end'] - densities['trunk']['start'])
                    density = densities['trunk']['min'] + slope * (distance - densities['trunk']['start'])
                    self.append_synapse_locs(node, 'exc', density)
        if 'apical' in sec_type_list:
            for node in self.apical:
                distance = self.get_distance_to_node(self.tree.root, self.get_dendrite_origin(node), loc=1.)
                slope = (densities['apical']['max'] - densities['apical']['min']) / \
        ...
        if 'tuft' in sec_type_list:
            for node in self.tuft:
                if self.is_terminal(node):
                    self.append_synapse_locs(node, 'exc', densities['tuft']['terminal'])
                else:
                    self.append_synapse_locs(node, 'exc', densities['tuft']['parent'])
        self._reinit_mech(self.spine)
    ...
    def insert_spine(self, node, parent_loc, child_loc=0):
        ...
        head.sec.diam = 0.5
        head.synapse_locs('exc').append(0.5)
        self._init_cable(head)

    def mark_inhibitory_synapse_locs(self, sec_type_list=None):
        """
        ...
        if 'soma' in sec_type_list:
            for node in self.soma:
                self.append_synapse_locs(node, 'inh', densities['soma'])
        if 'ais' in sec_type_list:
            for node in self.get_nodes_of_subtype('ais'):
                self.append_synapse_locs(node, 'inh', densities['ais'])
        if 'basal' in sec_type_list:
            for node in self.basal:
                if self.is_terminal(node):
                    self.append_synapse_locs(node, 'inh', densities['basal']['terminal'])
                else:
                    order = self.get_branch_order(node)
                    if order == 1:
                        self.append_synapse_locs(node, 'inh', densities['basal']['primary'])
                    else:
                        self.append_synapse_locs(node, 'inh', densities['basal']['intermediate'])
        if 'trunk' in sec_type_list:
            for node in self.trunk:
                distance = self.get_distance_to_node(self.tree.root, node)
                if distance >= densities['trunk']['start']:
                    slope = (densities['trunk']['max'] - densities['trunk']['min']) / \
                            (densities['trunk']['end'] - densities['trunk']['start'])
                    density = densities['trunk']['min'] + slope * (distance - densities['trunk']['start'])
                    self.append_synapse_locs(node, 'inh', density)
        if 'apical' in sec_type_list:
            for node in self.apical:
                distance = self.get_distance_to_node(self.tree.root, self.get_dendrite_origin(node), loc=1.)
                slope = (densities['apical']['max'] - densities['apical']['min']) / \
        ...
        if 'tuft' in sec_type_list:
            for node in self.tuft:
                if self.is_terminal(node):
                    self.append_synapse_locs(node, 'inh', densities['tuft']['terminal'])
                else:
                    self.append_synapse_locs(node, 'inh', densities['tuft']['parent'])

    def append_synapse_locs(self, node, category, density):
        """
